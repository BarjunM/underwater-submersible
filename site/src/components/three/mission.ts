...
export const COLOUR = {
  /*
   * Two colours and nothing else: the pipe, and the damage on it.
   *
   * The collars are gone. Five bands around a bar a few dozen pixels tall
   * were detail at a size that cannot carry detail — seen off-axis each one
   * drew as an arc, and quieting them until the arcs stopped reading left
   * marks doing no work. What is left is one clean form.
   */
  pipe: { light: '#0d0b09', dark: '#2f302b' },
  /*
   * Damage in red, which is the one thing on this page allowed to be loud.
   *
   * It was bone, and before that the site accent — both of which read as
   * markings on the pipe rather than as something wrong with it. A crack is
   * the whole reason the machine is down there; it should be the first thing
   * the eye lands on in the strip. Bright enough to hold at a hairline's
   * width against a near-black pipe on either theme.
   */
  crack: '#ff4530',
  /** The bead left behind, cooling from the crack colour as it sets. */
  weld: '#8a5a4a',
  hull: '#ecebe6',
}
